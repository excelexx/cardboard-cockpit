...
Camera indices above are examples; use the listing for your Mac. In the game choose **Set up cardboard**. Tracking enables automatically. Hold the yoke upright for three seconds, then follow the throttle idle/full/idle checks, yoke overview, shooting grip and gun cover/uncover checks. Each step advances when fresh camera observations confirm the action. At the end, centre the yoke, idle the throttle and turn off the gun for one second to begin the flight. **Retry this step** repeats a check; **Back / Escape / badge HOME** cancels setup; **Use keyboard** starts without the cameras. Camera Play and Tutorial both open this same guided setup before takeoff. The laptop handles yoke ID 7 and weapons. The phone handles three coplanar throttle tags: ID 0 at idle, ID 1 on the moving handle, ID 2 at full. The relative throttle does not need saved endpoints. Gun tag ID 4 fires while visible; the existing two-sided switches (31/32 and 41/42) remain supported. Keyboard T still controls missiles when using the gun-only tag.
