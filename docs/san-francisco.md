...
Source tree instances on neutral pavement/roof pixels or blue water are culled using the original aerial RGB pixels. Retained tree positions and meshes are unchanged; the original complete TreeList archives remain included. This prevents generic urban vegetation from covering photographed piers and airport pavement. `tools/filter_sf_vegetation.py` reproduces the selection.
