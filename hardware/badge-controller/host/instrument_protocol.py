...
EXTENDED=struct.Struct('<BBhhhHBBiihhh')
CONTACT_V2=struct.Struct('<hhBBhhh')
MAX_CONTACTS=12
...
def validate(data):
 if not isinstance(data,dict) or data.get('kind')!='instrument' or type(data.get('version')) is not int or data.get('version') not in (1,2):raise ValueError('Unknown snapshot')
 ...
 if isinstance(detail,bool) or not isinstance(detail,int) or not 0<=detail<=255:raise ValueError('Invalid presentation detail')
 if data['version']==2:
  for key,lo,hi in [('aim_yaw',-180,180),('aim_pitch',-90,90),('engine',0,100),('systems',0,7),('fpa_yaw',-180,180),('fpa_pitch',-90,90),('climb',-32767,32767),('seconds',0,65535),('accuracy',0,255),('throttle',0,100),('world_x',-10000000,10000000),('world_z',-10000000,10000000),('g_load',-20,20)]:number(data,key,lo,hi)
  if int(data['systems'])!=data['systems']:raise ValueError('Fractional systems flags')
  if data['accuracy']>100 and data['accuracy']!=255:raise ValueError('Invalid accuracy')
 name=data.get('name')
 ...
 for c in contacts:
  ...
  if data['version']==2:
   for key in ('vx','vy','altitude'):number(c,key,-32767,32767)
...
def encode(data,sequence):
 d=validate(data)
 b=HEADER.pack(b'SI',d['version'],int(d['mode']),int(d['flags']),len(d['contacts']),sequence&65535,
  round(d['roll']*100),round(d['pitch']*100),round(d['heading']*100),round(d['speed']),round(d['altitude']),
  int(d['score']),int(d['kills']),int(d['pilot']),int(d['landing']),d['name'].encode().ljust(12,b'\0'),round(d['range']),d.get('detail',0))
 if d['version']==2:b+=EXTENDED.pack(round(d['engine']),int(d['systems']),round(d['fpa_yaw']*100),round(d['fpa_pitch']*100),round(d['climb']),round(d['seconds']),round(d['accuracy']),round(d['throttle']),round(d['world_x']),round(d['world_z']),round(d['g_load']*100),round(d['aim_yaw']*100),round(d['aim_pitch']*100))
 for c in d['contacts']:
  b+=CONTACT.pack(round(c['x']),round(c['y']),int(c['kind']),int(c['selected']))
  if d['version']==2:b+=struct.pack('<hhh',round(c['vx']),round(c['vy']),round(c['altitude']))
...
def fragments(packet,sequence,mtu_payload):
 ...
 if not 46<=len(packet)<=216:raise ValueError('Bad frame length')
