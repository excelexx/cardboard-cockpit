...
func run() -> void:
	...
	app.controls_lesson.index=6;await snapshot("setup-gear")
	app.controls_lesson.index=7;await snapshot("setup-landing")
	app.controls_lesson.index=app.controls_lesson.STEPS.size();await snapshot("setup-ready")
	app.cancel_control_setup();app.start_flight("demo");app.vision.enabled=false;app.camera_rig.update(1);await snapshot("coach-takeoff")
	app.flight.spawn_airborne(Vector3(0,350,-800),110);app.flight.gear=true;app.flight.flaps=1;app.camera_rig.update(1);await snapshot("coach-retract")
	app.flight.gear=false;app.flight.flaps=0;await snapshot("coach-cruise")
	app.landing_started=true;app.flight.gear=false;app.flight.flaps=0;app.flight.throttle=.7;await snapshot("coach-landing-clean")
	app.flight.gear=true;app.flight.flaps=2;app.flight.throttle=.2;await snapshot("coach-landing-configured")
