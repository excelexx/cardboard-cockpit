...
func run_tests() -> void:
	...
	app.combat.reset(true);app.combat.spawn_contact()
	...
	for i in range(12000):
		...
		if i%60==0 and app.flight.position.y<60: print("DBG t=",i/60.0," y=",app.flight.position.y," pitch=",rad_to_deg(app.flight.pitch)," aoa=",rad_to_deg(app.flight.aoa)," vs=",app.flight.vertical_speed," spd=",app.flight.speed," mode=",app.mode," contact=",app.flight.contact)
	print("DBG END mode=",app.mode," contact=",app.flight.contact," speed=",app.flight.speed," y=",app.flight.position.y," pitch=",rad_to_deg(app.flight.touchdown_pitch)," sink=",app.flight.touchdown_sink," success=",app.mission_success," reason=",app.result_reason)
