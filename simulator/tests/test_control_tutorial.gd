...
func run() -> void:
	...
	app.on_action("approach")
	check(app.flight.airborne, "Explicit landing practice begins on final approach")
	app.on_action("title")
	app.on_action("fly")
	check(app.flight_kind == "demo", "Play after landing practice selects a runway departure")
	check(app.mode == "tutorial" and app.vision.enabled and not app.copilot, "Play enables camera and enters tutorial")
	var first_request: String = app.vision.yoke_calibration.request_id
	app.on_action("tutorial_retry")
	check(app.tutorial.calibrating and app.vision.yoke_calibration.request_id != first_request, "Retry during calibration starts a new full capture")
	app.on_action("tutorial_start")
	check(app.mode == "tutorial", "Start flight action cannot bypass unfinished checks")
	...
	app._input(click)
	check(app.flight.position == position and app.flight.elapsed == 0 and app.combat.rounds_fired == 0, "Tutorial freezes flight and blocks camera, keyboard and mouse firing")
	app.tutorial = lesson; app.vision = v
	var frame := Image.create(20,20,false,Image.FORMAT_RGB8)
	app.vision_preview.texture = ImageTexture.create_from_image(frame)
	v.last_received = Time.get_ticks_msec()-200
	app.update_tutorial(.016)
	check(app.mode == "tutorial", "Automatic start revalidates tracking freshness")
	observe(lesson, v, 40)
	v.last_received = Time.get_ticks_msec(); lesson.last_sample = v.last_received
	app.update_tutorial(.016)
	check(app.mode == "flight" and not app.copilot and app.flight.throttle == 0 and app.vision_preview.texture == null, "Ready tutorial automatically starts camera flight at tested idle power and closes preview")
	check(not app.flight.airborne and app.flight.speed==0 and app.is_on_runway(app.flight.position),"Completing the tutorial starts on the runway")
	app.on_action("title"); app.on_action("fly")
	check(app.tutorial.calibrating and app.tutorial.index == 0 and not app.tutorial.can_start and app.vision.yoke_calibration.active, "Next Play begins a fresh calibration before sequential checks")
	press.keycode = KEY_ESCAPE; app._input(press)
	check(app.mode == "title" and not app.vision.yoke_calibration.active, "Escape returns to menu and cancels any unfinished calibration")
	app.on_action("keyboard_play")
	check(app.mode == "flight" and not app.vision.enabled and app.copilot, "Explicit keyboard choice remains available without camera")
