...
func run()->void:
	...
	var direct:=bird(at);var near:=bird(at+Vector3(10,0,0));var edge:=bird(at+Vector3(30,0,0));var capped:=bird(at+Vector3(32,0,0));var outside:=bird(at+Vector3(36,0,0))
	app.combat.spawn_shot(at,Vector3(0,0,-180),"missile",direct.id,110);var shot:Dictionary=app.combat.shots.back()
	var hull:float=app.combat.hull
	app.combat.detonate_missile(shot,direct.id)
	check(direct.health==0 and near.health>=75 and edge.health>=75 and app.combat.kills==1,"One missile kills its direct target while healthy neighbours survive for plasma")
	check(edge.health>near.health and edge.health<100,"Light splash damage falls off toward the edge")
	check(capped.health==100 and outside.health==100,"Splash is limited to two neighbours within 35 metres")
	...
	app.combat.detonate_missile(shot,direct.id)
	check(edge.health==before and app.combat.score==score and app.combat.kills==1,"The same missile cannot detonate or score twice")
	app.combat.spawn_shot(at,Vector3(0,0,-180),"missile",edge.id,110);app.combat.detonate_missile(app.combat.shots.back())
	check(app.combat.kills==1 and near.health>0 and edge.health>0,"Overlapping splash does not wipe out healthy nearby geese")
	app.start_flight("combat");app.flight.spawn_airborne(Vector3(0,3000,0),180);app.combat.spawn_clock=999
