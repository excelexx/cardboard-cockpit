...
func run() -> void:
	...
	check(app.mode=="flight" and app.landing_recovery and app.flight.contact=="" and app.flight.airborne,"Off-airport mishap starts an automatic return instead of accepting touchdown")
	app.flight.position=Vector3(0,3,0);app.settle_demo_touchdown()
	check(app.mode=="rollout" and app.flight.contact=="landed" and not app.landing_recovery,"Airport touchdown completes the automatic return")
