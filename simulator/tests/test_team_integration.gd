...
func run() -> void:
	...
	check(app.gameplay_settings.pitch_agility==1 and app.gameplay_settings.bank_agility==1 and app.gameplay_settings.yaw_agility==1,"Unchanged default handling")
	...
	app.on_action("sensitivity_reset");check(app.flight.pitch_agility==1 and app.combat.aim_strength==1,"Reset keeps this branch's neutral handling")
	...
	app.on_action("training");check(app.flight_kind=="training" and app.mission is TrainingMission,"Separate takeoff/combat/landing tutorial")
	...
	app.on_action("fly");check(app.flight_kind=="demo" and not app.mission is TrainingMission,"Normal Play remains this branch's mission")
	check(app.combat.training_target_limit==-1,"Normal combat limit restored")
	# Both camera formats reach the actual latch handler; gun-only tags cannot own missiles.
	app.vision.enabled=false
	var camera_packet: Dictionary={"version":1,"sequence":10,"timestamp":100,"tracking":true,"yoke":{"roll":0,"pitch":0,"yaw":0,"confidence":1},"throttle":{"value":.4,"confidence":1},"weapons":{"primary":true,"salvo":true,"primary_confidence":1,"salvo_confidence":1}}
	check(app.vision._accept_packet(JSON.stringify(camera_packet).to_utf8_buffer(),100),"Legacy two-switch packet accepted")
	app._process(0)
	check(app.primary_latched and app.salvo_latched,"Legacy packet controls both current weapon systems")
	camera_packet.sequence=11;camera_packet.weapons={"gun":false}
	check(app.vision._accept_packet(JSON.stringify(camera_packet).to_utf8_buffer(),110),"Gun-tag packet accepted")
	app._process(0)
	check(not app.primary_latched and app.salvo_latched,"Gun-tag packet leaves keyboard missile latch intact")
	camera_packet.sequence=12;camera_packet.weapons={"primary":false,"salvo":false,"primary_confidence":1,"salvo_confidence":1}
	app.vision._accept_packet(JSON.stringify(camera_packet).to_utf8_buffer(),120);app._process(0)
	check(not app.primary_latched and not app.salvo_latched,"Legacy switches can turn both weapons off")
