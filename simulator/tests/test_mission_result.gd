...
func run() -> void:
	...
	app.flight.contact="landed";app.flight.speed=0
	...
	var state: Dictionary={"cinematic":true,"cleared":true,"down":12,"total":12,"contact":"landed","stopped":true}
	var result: Dictionary=assess.assess(state,true,"")
	wrong=wrong or not result.success or result.code!="mission_complete"
	state.cleared=false;state.down=4;state.early_landing=true;result=assess.assess(state,true,"")
	wrong=wrong or result.success or result.code!="skein_incomplete" or not result.advice.contains("before") or not result.summary.contains("8")
	# Near miss plus a landing is still a win at the booth.
	state.down=9;result=assess.assess(state,true,"")
	wrong=wrong or not result.success or result.code!="mostly_clear"
	state.down=8;result=assess.assess(state,true,"")
	wrong=wrong or result.success or result.code!="skein_incomplete"
	state.cleared=true;state.down=12;state.contact="";state.stopped=false;result=assess.assess(state,true,"")
	wrong=wrong or result.success or result.code!="recovery_incomplete"
	state.cleared=false;state.down=3;result=assess.assess(state,true,"")
	wrong=wrong or result.success or result.code!="mission_incomplete" or not result.summary.contains("3 of 12")
