...
func run() -> void:
	...
	app.combat.fire_primary();app.combat.update_beam(.1)
	check(app.combat.beam_target_ids==[int(a.id),int(a.id)],"Both beams focus one goose at a time")
	...
	check(is_equal_approx(a.health,100-Tune.BEAM_DPS*.2) and is_equal_approx(b.health,100),"Focused beams drain one health ring")
	app.combat.hurt_enemy(a,1000,"beam",a.position);app.combat.update_beam(.1)
	check(app.combat.beam_target_ids[0]==b.id and app.combat.beam_target_ids[1]==b.id and is_equal_approx(b.health,100-Tune.BEAM_DPS*.2),"A lone remaining goose receives both cannons at doubled combined damage")
	...
	app.combat.fire_primary();app.combat.update_beam(.1)
	check(app.combat.beam_target_ids==[int(a.id),int(a.id)] and is_equal_approx(a.health,400-Tune.BEAM_DPS*.2) and b.health==100,"Both cannons focus a tougher central target")
	...
	fresh()
	for i in range(6):bird(Vector3((i-2.5)*45,0,-700))
	for frame in range(720):app.combat.spawn_clock=INF;app.combat.fire_primary();app.combat.tick(1.0/60)
	check(app.combat.kills==6,"The same flock is feasible to clear with dual plasma in twelve seconds")
	fresh()
