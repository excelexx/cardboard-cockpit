...
const Fighter = preload("res://scenes/fighter_cockpit.gd")
var canopy: FighterCanopy
var fighter: FighterCockpit
...
func build(aircraft: Dictionary) -> void:
	...
	if is_instance_valid(_active_root):
		_interior_cache[airframe]={"root":_active_root,"displays":displays.duplicate(),"throttles":throttles.duplicate(),"pilot":pilot_control,"rest":control_rest,"stick":control_is_stick,"fighter":fighter,"canopy":canopy}
	...
	if _interior_cache.has(requested):
		...
		fighter=cached.get("fighter",null)
		canopy=cached.get("canopy",null)
	...
	if airframe=="f35":
		# The fighter interior is built from scratch in cockpit space by
		# scenes/fighter_cockpit.gd. The sourced FlightGear tub it replaced was a
		# handful of thin planes with no sidewalls, seat, pedestal or pilot, which
		# is why the view read as a flat painting instead of an interior.
		var interior := Fighter.new()
		interior.name = "FighterInterior"; interior.set_meta("cached_interior",true)
		_active_root = interior
		add_child(interior)
		var live_panel := _instrument_texture("panorama")
		for viewport: Node in get_children():
			if viewport is SubViewport: viewport.reparent(interior)
		interior.build(profile,live_panel)
		fighter = interior
		canopy = interior.canopy
		pilot_control = interior.stick_pivot
		control_rest = pilot_control.position
		throttles.append(interior.throttle_pivot)
		return
...
func set_presentation_visible(value: bool) -> void:
	...
	for light: Node in _active_root.find_children("*","Light3D",true,false):
		light.visible = value
...
func update_instruments(flight: FlightDynamics, control: Vector3, delta: float) -> void:
	...
	for lever: Node3D in throttles:
		lever.rotation.x = lerpf(0.46,-0.44,flight.throttle)
	if is_instance_valid(fighter): fighter.set_attitude(flight.pitch,flight.roll)
...
func set_sun(direction: Vector3,energy: float) -> void:
	var local: Vector3 = global_basis.inverse()*direction
	if is_instance_valid(fighter):
		fighter.set_sun(local,energy)
	elif is_instance_valid(canopy):
		canopy.set_sun(local,energy)
