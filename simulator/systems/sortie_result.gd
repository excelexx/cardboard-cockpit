...
static func assess(s: Dictionary,requested_success: bool,reason: String) -> Dictionary:
	...
	if s.get("tutorial",false):
		if landed and s.get("training_ready",false):return make("training_complete",true,"TRAINING COMPLETE","Weapons demonstrated; aircraft landed and stopped.","Ready for Play: defeat the geese and boss, then land at SFO.")
		return make("training_incomplete",false,"TRAINING INCOMPLETE","The required exercises are not finished.","Restart the tutorial and complete each weapon and landing exercise.")
