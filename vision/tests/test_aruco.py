...
class ArucoTests(unittest.TestCase):
    ...
    def test_active_detector_uses_pose_guard_without_losing_relative_throttle(self):
        frame = throttle_frame(.5, yoke=True)
        with patch.object(self.tracker.pose_guard, "accept", return_value=False) as accept:
